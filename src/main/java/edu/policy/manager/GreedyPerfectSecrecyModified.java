...
public class GreedyPerfectSecrecyModified extends GreedyAlgorithm {

    private static final Logger logger = LogManager.getLogger(GreedyPerfectSecrecyModified.class);

    public GreedyPerfectSecrecyModified(Session session) {
        super(session);
    }
    ...
    public void greedyPerfectDen(List<Cell> senCells) {
        ...
        // Hide addition cells
        List<Cell> hideCellsAdditional = new ArrayList<>();
        for (Cell cell: hideCells) {
            int tid = cell.getTupleID();
            for(Cell senCell: senCells) {
                String senAttr = senCell.getAttributeName();
                Cell newCell = new Cell(senCell.getDatabaseName(), senCell.getRelationName(), senAttr, tid);
                newCell.setCellType(senCell.getCellType());

                switch (newCell.getCellType()) {
                    case STRING:
                        newCell.setAttrDomSize((int) senCell.getAttrDomSize());
                        break;
                    case INTEGER:
                    case DOUBLE:
                        newCell.setMinMaxDom(senCell.getMinDomain(), senCell.getMaxDomain());
                        break;
                }

                hideCellsAdditional.add(newCell);
            }
        }

        hideCells = Utils.unionCells(hideCells, hideCellsAdditional);
