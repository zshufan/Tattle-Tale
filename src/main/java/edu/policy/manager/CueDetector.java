...
public class CueDetector {
    ...
    private String multiQueryGeneration(DataDependency dependency) {
        ...
        List<OperationType> operationTypeList = dependency.getSchemaPredicates().stream().map(Predicate::getOperation).collect(toList());


        List<OperationType> rangeOperators = new ArrayList<>(Arrays.asList(OperationType.GTE, OperationType.GT, OperationType.LT, OperationType.LTE));
        if (intersection(operationTypeList, rangeOperators).isEmpty()) {
            // The dependency is not a DC with range operators
            List<Predicate> predicates = dependency.getSchemaPredicates();
            for (Predicate pred: predicates) {
                conds.append("(T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(" is null OR T1.").append(Utils.escapeReserved(pred.getAttributeName()));
                if (pred.getOperation().equals(OperationType.EQ))
                    conds.append("=").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");
                else if (pred.getOperation().equals(OperationType.NEQ))
                    conds.append("<>").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");

                if (predicates.indexOf(pred) != (predicates.size() - 1))
                    // if not the last predicate
                    conds.append("AND ");
                else
                    conds.append(")");
            }
            ...
            // The dependency has range operators
            ...
            for (Predicate pred: predicates) {
                ...
                // range operators
                if (pred.getOperation().equals(OperationType.LT))
                    conds.append(">").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");
                else if (pred.getOperation().equals(OperationType.LTE))
                    conds.append(">=").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");
                else if (pred.getOperation().equals(OperationType.GT))
                    conds.append("<").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");
                else if (pred.getOperation().equals(OperationType.GTE))
                    conds.append("<=").append("T2.").append(Utils.escapeReserved(pred.getAttributeName())).append(") ");
